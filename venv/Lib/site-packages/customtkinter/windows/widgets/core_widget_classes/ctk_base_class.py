...
class CTkBaseClass(tkinter.Frame, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    ...
    def _check_image_type(self, image: any):
        """Check image type when passed to widget and warn if not CTkImage"""
        if image is None:
            return None
        ...
        try:
            ...
            warnings.warn(
                f"{widget_type} Warning: Given image is not CTkImage but {image_type}. "
                "Image cannot be scaled on HighDPI displays. Use CTkImage instead.",
                stacklevel=2
            )
        except Exception as e:
            warnings.warn(f"Image type check failed: {e}", stacklevel=2)
